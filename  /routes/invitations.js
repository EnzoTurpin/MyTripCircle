...
// PUT /invitations/:token — accepter/refuser
router.put("/:token", async (req, res) => {
  try {
    ...
    if (invitation.type === "link") {
      ...
      if (!userId) return res.status(400).json({ error: "userId requis pour rejoindre via lien" });
    ...
    if (action === "accept") {
      ...
      await db.collection("trips").updateOne(
        { _id: new ObjectId(invitation.tripId) },
        { $push: { collaborators: { userId: userId || invitation.inviteeEmail, role: permissions.role, joinedAt: new Date(), permissions, invitedBy: invitation.inviterId } } }
      );
    }
