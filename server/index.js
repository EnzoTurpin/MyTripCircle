...
connectMongo()
  .then(() => {
    ...
    const bindHost = process.env.NODE_ENV === "production" ? "0.0.0.0" : ACTIVE_IP;
    app.listen(PORT, bindHost, () => {
      console.log(`[server] API listening on http://${bindHost}:${PORT}`);
      console.log(`[server] Accessible via http://localhost:${PORT}`);
      if (bindHost !== "0.0.0.0") {
        console.log(`[server] Use this IP in your app: ${bindHost}`);
      }
    });
  })
  .catch((err) => {
