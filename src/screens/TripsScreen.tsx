...
const TripsScreen: React.FC = () => {
  ...
  // Ref pour la FlatList afin de préserver la position de défilement
  const flatListRef = useRef<FlatListType<Trip>>(null);
  const scrollPosition = useRef<number>(0);
  const hasLoadedInitially = useRef<boolean>(false);

  // Rafraîchir les données uniquement au premier chargement (pas au retour du détail)
  useFocusEffect(
    useCallback(() => {
      if (!hasLoadedInitially.current) {
        ...
        // Au retour d'un autre écran, restaurer la position de défilement
        setTimeout(() => {
  ...
  const handleTripPress = (trip: Trip) => {
    // Sauvegarder la position de défilement actuelle avant de naviguer
    // La position est déjà capturée par onScroll, on navigue simplement
    navigation.navigate("TripDetails", { tripId: trip.id });
  };

  // Capturer la position de défilement lors du scroll
